...
def gerar_processos(qtd_processos, chegada, burst):
    processos = []  # Lista que irá armazenar os processos gerados
    tempo_atual = 0  # Tempo de chegada do primeiro processo é 0

    # Se uma lista estática for fornecida, use-a
    if chegada == "poisson":
    
        if burst == " normal":
            return
        elif burst == "exponencial":
            return
    
    elif chegada == "exponencial":
        ...
        # Se não houver lista estática, gera os processos aleatoriamente
